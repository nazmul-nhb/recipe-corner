...
const Recipe = ({ recipe, handleWantToCook }) => {
    ...
    return (
        <div className='flex flex-col justify-between gap-3 md:gap-4 p-4 md:p-6 border border-[#28282833] rounded-2xl'>
            ...
            <h4 className="text-lg md:mt-6">Ingredients: {ingredients.length}</h4>
            <ul className="text-base md:text-lg text-[#878787] fira-sans md:mb-6">
                {
                    ingredients.map(
                        (ingredient, idx) =>
            ...
            <button onClick={() => handleWantToCook(recipe)} className="w-full md:w-44 h-12 flex items-center justify-center text-[#150B2B] rounded-[50px] font-semibold text-lg bg-[#0BE58A] border border-[#0BE58A] hover:text-[#0BE58A] hover:border-[#0BE58A] hover:bg-transparent">Want to Cook</button>
        </div>
    );
};
